console.log('PerfumeDirect - Ultimate Responsive Shopify Theme. Version 1.0 by Tidal Digital')
...
function freeShipping(current, max) {
    ...
    $current.html(debute.Currency.formatMoney((max - current) * 100, js_helper.moneyFormat));
    $max.html(max);
...
function modalCheckoutUpdate() {
    if ($('body').hasClass('checkout-popup')) {
        ...
        $('.js-mdlchk-prd-total').html(debute.Currency.formatMoney(CartJS.cart.total_price, js_helper.moneyFormat));
...
function renderCartInModalPopup() {
    ...
    $.each(CartJS.cart.items, function (index, item) {
        ...
        $item = '<div class="cart-table-prd"> <div class="cart-table-prd-image"> <a href="' + item.url + '"><img src = "' + item.image + '" alt="' + item.product_title + '"></a> </div> <div class="cart-table-prd-name"> <h5><span>' + item.vendor + '</span></h5> <h2><a href="' + item.url + '">' + item.title + '</a></h2></div> <div class="cart-table-prd-price"><b>' + debute.Currency.formatMoney(item.price, js_helper.moneyFormat) + '</b></div> <div class="cart-table-prd-qty"> <div class="prd-block_qty"> <div class="qty qty-changer"> <fieldset> <input type="button" value="&#8210;" class="decrease" data-variant-id="' + item.variant_id + '" data-variant-url="' + item.url + '" data-variant-name="' + item.title + '"> <input disabled type="text" class="qty-input" value="' + item.quantity + '" data-min="0" data-max="9999"> <input type="button" value="+" class="increase" data-variant-id="' + item.variant_id + '" data-variant-url="' + item.url + '" data-variant-name="' + item.title + '"> </fieldset> </div> </div> </div> <div class="cart-table-prd-price"><b>' + debute.Currency.formatMoney(item.line_price, js_helper.moneyFormat) + '</b></div> <div class="cart-table-prd-action"> <a href="' + item.url + '" data-variant-name="' + item.title + '" data-variant-id="' + item.variant_id + '" data-line-number="' + line_item + '"  title="' + locales.remove + '" class="icon-cross js-popupcart-remove-item"></a> </div> </div>';
    ...
    if (CartJS.cart.item_count > 0) {
        $('.btn.checkout_procees').show()
    } else {
        $('.btn.checkout_procees').hide()
    }
    currencyUpdate();
}
    ...
    setTimeout(function () {
        ...
        $.each(CartJS.cart.items, function (i, item) {
            if (item.id == variant_id) {
                $('.modalchk-price').html(debute.Currency.formatMoney(item.price, js_helper.moneyFormat));
...
function _changeVprice(price, compare_price, el) {
    $('.price-new', el).html(debute.Currency.formatMoney(price, moneyFormat));
    $('.prd-price .price-old,.prd-price .price-comment', el).remove();
    if (compare_price) {
        $('<div class="price-old">' + debute.Currency.formatMoney(compare_price, moneyFormat) + '</div><div class="price-comment"></div>').appendTo($('.prd-price', el));
    }
    currencyUpdate();
}

function _changeQuickViewPrice(price, compare_price, el) {
    $('.prd-block_price--actual', el).html(debute.Currency.formatMoney(price, moneyFormat));
    alert('B');
    $('.prd-block_price--old', el).remove();
    if (compare_price) {
        $('<div class="prd-block_price--old">' + debute.Currency.formatMoney(compare_price, moneyFormat) + '</div>').appendTo($('.prd-block_price', el));
    }
    currencyUpdate();
}
...
function updateData($type, $id, $options) {
    ...
    if (eval(path) != undefined) {
        ...
        /*price changer*/
        // $('.prd-block_price--actual', product_to_update).html(debute.Currency.formatMoney(eval(path + '[\'price\']'), js_helper.moneyFormat));
        $('.prd-block_price--actual', product_to_update).html('<div class="arsssss">' + debute.Currency.formatMoney(eval(path + '[\'price\']'), js_helper.moneyFormat) + '</div>');
        // $('.prd-block_price--actual', product_to_update).addClass('arslan');
        ...
        if (eval(path + '[\'compare_at_price\']') != '') {
            if (parseInt(eval(path + '[\'compare_at_price\']')) > parseInt(eval(path + '[\'price\']'))) {
                if ($('.prd-block_price').hasClass('show_you_save')) {
                    ...
                    $('.prd-block_price .price-comment span').html(debute.Currency.formatMoney(save, js_helper.moneyFormat));
                }

                if ($('.prd-block_price .prd-block_price--old').length) {
                    $('.prd-block_price .prd-block_price--old', product_to_update).html(debute.Currency.formatMoney(eval(path + '[\'compare_at_price\']'), js_helper.moneyFormat));
                } else {
                    $('.prd-block_price', product_to_update).append('<div class="prd-block_price--old">' + debute.Currency.formatMoney(eval(path + '[\'compare_at_price\']'), js_helper.moneyFormat) + '</div>');
                }
            } else {
...
function cartPopupUpdate() {
    ...
    if (CartJS.cart.item_count > 0) {
        ...
        if (checkout_popup_free_shipping_yes && free_shipping_header_cart_yes) {
            free_shipping = '<div class="free-shipping">' +
                ...
                '   </div>' +
                '   <div class="free-shipping-text js-free-shipping-text" data-count="' + checkout_popup_free_shipping_count + '">' + locales.checkout_popup_free_condition_text + ' (' + locales.free_shipping_from + '  ' + debute.Currency.formatMoney(checkout_popup_free_shipping_count * 100, js_helper.moneyFormat) + ').</div>' +
        ...
        $('.minicart-drop-content').html('<h3>' + locales.recently_added_items + ':</h3><div class="cart-list-prd"></div>' + free_shipping +
            ...
            '       <span class="minicart-drop-summa"><span>' + locales.subtotal + ':</span> <b>' + debute.Currency.formatMoney(CartJS.cart.total_price, js_helper.moneyFormat) + '</b></span> ' +
        ...
        $.each(CartJS.cart.items, function (index, item) {
            ...
            $item = '<div class="minicart-prd"> <div class="minicart-prd-image"> <a href="' + item.url + '" title="' + item.product_title + '"><img src = "' + item.image + '" alt="' + item.product_title + '"> </a> </div> <div class="minicart-prd-name"> <h5><span>' + item.vendor + '</span></h5> <h5><a href="' + item.url + '">' + variant_title + '</a></h5> <h2><a href="' + item.url + '">' + item.product_title + '</a></h2> </div> <div class="minicart-prd-qty"><span>' + locales.qty + ':</span> <b>' + item.quantity + '</b></div> <div class="minicart-prd-price"><span>' + locales.price + ':</span> <b>' + debute.Currency.formatMoney(item.price, js_helper.moneyFormat) + '</b></div> <div class="minicart-prd-action"> <a href="' + item.url + '"  class="icon-pencil "></a> <a href="' + item.url + '" data-variant-id="' + item.variant_id + '" data-line-number="' + line_item + '"  title="' + locales.remove + '" class="icon-cross js-minicart-remove-item"></a> </div> </div>';
            $updated_list = $updated_list + $item;
            line_item = line_item + 1;
        });
        $(cart_list).html($updated_list);
    ...
    $cart_subtotal.html(debute.Currency.formatMoney(CartJS.cart.total_price, js_helper.moneyFormat));
...
debute.Product = (function () {
    function Product(container) {
        ...
        this.selectors = {
            ...
            youSaveAmount: '[data-you-save] span:last'
        };
    ...
    Product.prototype = _.assignIn({}, Product.prototype, {
        ...
        _updatePrice: function (evt) {
            ...
            if (variant.compare_at_price > variant.price) {
              ...
              var varienttitle = variant.title;
              if(variant.price >= 5000){
                $regularPrice.html(
                    debute.Currency.formatMoney(variant.price, js_helper.moneyFormat)
                );
                $regularPrice.append('<span class="prd-label--sale prd-label--freeshipping">FREE SHIPPING</span>');
                
                data = $.parseJSON($badge);
